...
const precioComponente = (componente) => {
  ...
  const precioMaquina = (componentes)=>{
    ...
    for (const componente of componentes){
        precioComp += precioComponente(componente)
    }
    return precioComp
}
...
const contadorVentas = (ventas) =>{
    ...
    for(let venta of ventas){
        acc += precioMaquina(ventas.componentes)
    }
    return acc
}
...
const ventasVendedoras = (vendedora) =>{
    contadorVentas(ventas.filter (ventas=> venta.nombreVendedora === vendedora))
}
//console.log(ventasVendedoras("Grace"))
...
const huboVentas = (mes,anio) => vendedoraDelMes(mes,anio).length > 0
...
const sucursalDelMes = (mes,anio) =>{
 ...
 for (const sucursal of sucursales){
     if(acc < (contadorVentas (ventasDelMes(mes,anio).filter(ventas => ventas.sucursal === sucursal)))){
         acc = (contadorVentas (ventasDelMes(mes,anio).filter(ventas => ventas.sucursal === sucursal)))
         succDelMes = sucursal
     }
 }
 return succDelMes
